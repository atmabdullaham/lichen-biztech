...
export default function HeroSection() {
  ...
  const serviceItems = useMemo(() => [
    { href: "/services/software",            icon: Code2,     title: "Software & Tech",      desc: "Custom software, web & mobile apps, ERP, CRM systems" },
    { href: "/services/print",               icon: Palette,   title: "Design & Print",        desc: "Digital printing, offset, apparel, packaging design" },
    { href: "/services/marketing",           icon: Megaphone, title: "Marketing",             desc: "Social media, content creation, brand identity" },
    { href: "/services/consultancy",         icon: TrendingUp,title: "Consultancy",           desc: "Brand consultancy, market research, strategy" },
    { href: "/services/creative-manager",    icon: Zap,       title: "Creative Manager",      desc: "Monthly expert management for content & automation" },
    { href: "/services/business-consultancy",icon: BarChart3, title: "Business Consultancy",  desc: "Free consultancy for strategic planning" },
  ], []);

  // Set of premium client logos for the bottom notch marquee
  const marqueeLogos = useMemo(() => [
    // Logo 1: Stripe
    <div key="logo-1" className="flex items-center opacity-70 hover:opacity-100 transition-opacity duration-300">
      <svg className="w-20 h-7 sm:w-24 sm:h-8 text-[#1E5028]" viewBox="0 0 60 20" fill="currentColor">
    ...
    // Logo 2: Slack
    <div key="logo-2" className="flex items-center gap-1.5 opacity-70 hover:opacity-100 transition-opacity duration-300 text-[#1E5028]">
      ...
      <span className="text-[10px] md:text-xs font-black tracking-tight font-sans">slack</span>
    </div>,
    // Logo 3: Shopify
    <div key="logo-3" className="flex items-center gap-1.5 opacity-70 hover:opacity-100 transition-opacity duration-300 text-[#1E5028]">
      <svg className="w-7 h-7" viewBox="0 0 24 24" fill="currentColor">
      ...
      <span className="text-[10px] md:text-xs font-bold tracking-tight font-sans">shopify</span>
    </div>,
    // Logo 4: Airbnb
    <div key="logo-4" className="flex items-center gap-1.5 opacity-70 hover:opacity-100 transition-opacity duration-300 text-[#1E5028]">
      ...
      <span className="text-[10px] md:text-xs font-bold tracking-tight font-sans">airbnb</span>
    </div>,
    // Logo 5: Focus Coaching
    <div key="logo-5" className="flex items-center opacity-100 transition-opacity duration-300">
      <Image
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="absolute -bottom-40 -left-40 w-96 h-96 rounded-full bg-[#85C441]/8 blur-[100px] pointer-events-none animate-pulse-glow" style={{ animationDelay: "2.5s" }} />
                ...
                <NavigationMenuItem>
                  ...
                  <NavigationMenuContent className="right-[-120px] left-auto top-full mt-2.5 bg-white/95 dark:bg-[#0E2A14]/95 border border-[#85C441]/20 dark:border-white/10 shadow-xl shadow-black/10 dark:shadow-[0_0_30px_rgba(133,196,65,0.15)] rounded-xl backdrop-blur-md">
                    <ul className="grid w-[280px] gap-2 p-3 sm:w-[400px] md:w-[600px] md:grid-cols-[1.2fr_2fr] list-none">
                      {/* Left Sidebar Info */}
                      <li className="hidden sm:block sm:row-span-6 rounded-xl p-4" style={{ background: "var(--gradient-hero)" }}>
                        <div className="mb-2 mt-4 text-base font-bold text-white">
                          Lichen Ecosystem
                        </div>
                        <p className="mb-4 text-xs leading-relaxed text-white/60">
                          Explore everything you can build with our comprehensive 360° business solutions.
                        </p>
        ...
        <div className="relative z-10 max-w-4xl px-6 text-center flex flex-col items-center">
          ...
          >
            ...
            <br />
            <span
              className="animate-gradient-shift bg-clip-text text-transparent bg-gradient-to-r from-[#85C441] via-[#B8E986] to-[#85C441]"
            >
              Complete Solution
            </span>
          </motion.h1>
        ...
        >
          <div className="flex items-center gap-10 md:gap-14 animate-[marquee_16s_linear_infinite] w-max whitespace-nowrap">
            ...
            {marqueeLogos.map((logo, idx) => (
              <div key={`set1-${idx}`} className="shrink-0 flex items-center justify-center">
                {logo}
              </div>
            ))}
            {/* Duplicate Set */}
            {marqueeLogos.map((logo, idx) => (
              <div key={`set2-${idx}`} className="shrink-0 flex items-center justify-center">
                {logo}
              </div>
            ))}
          </div>
